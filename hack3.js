(async () => {
  ...
  // Open a blank window
  const blankWindow = window.open('', '_blank', 'width=820,height=750,scrollbars=yes,resizable=yes');
  if (!blankWindow) {
    alert('Popup blocked. Allow popups for this site and try again.');
    return;
  }

  // Write basic HTML structure to the blank window
  ...
  blankWindow.document.write(`
    ...
    <body>
      ...
      <div id="answersPopup" style="display: none;"></div>
    </body>
    </html>
  `);
  ...
  try {
    ...
    // Build the answers content for the popup DIV
    let contentHtml = `
    ...
    contentHtml += `
          <div class="popup-actions">
            <button onclick="this.closest('.popup-overlay').style.display='none'" class="close-btn">Close</button>
          </div>
        </div>
      </div>
    `;
    ...
    // Inject the popup content into the blank window
    blankWindow.document.getElementById('answersPopup').innerHTML = fullCss + contentHtml;
    blankWindow.document.getElementById('answersPopup').style.display = 'block';
    
    // Remove the loading screen
    blankWindow.document.body.innerHTML = blankWindow.document.getElementById('answersPopup').outerHTML;
    blankWindow.document.title = title;

  } catch (err) {
    // Show error in the blank window
    blankWindow.document.body.innerHTML = `
      <div style="color: white; text-align: center; padding: 40px; font-family: Inter, sans-serif;">
        <h2>Error Loading Answers</h2>
        <p>${err && err.message ? err.message : String(err)}</p>
        <button onclick="window.close()" style="margin-top: 20px; padding: 10px 20px; background: #667eea; color: white; border: none; border-radius: 8px; cursor: pointer;">Close</button>
      </div>
    `;
    console.error(err);
  }
